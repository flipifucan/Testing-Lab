...
- Elevate privileges with `sudo` and `su`; and
...
- [x] [02. Instructor Do: Users and Passwords](LessonPlan.md#02-instructor-do-users-and-passwords-020)
- [x] [03: Student Do: Talk to John](LessonPlan.md#03-student-do-talk-to-john-020)
- [x] [04: Instructor Review: Let's Talk to John](LessonPlan.md#04-instructor-review-lets-talk-to-john-010)
- [x] [05: Instructor Do: Privileges, root, sudo and su demo](LessonPlan.md#05-instructor-do-privileges-root-sudo-and-su-demo-020)
- [x] [06: Student Do: sudo Wrestling](LessonPlan.md#06-student-do-sudo-wrestling-025)
- [x] [07. Instructor Review: sudo Wrestling](LessonPlan.md#07-instructor-review-sudo-wrestling-010)
- [x] [08. Break](LessonPlan.md#08-break-015)
- [x] [09. Instructor Do: Users and Groups](LessonPlan.md#09-instructor-do-users-and-groups-020)
- [x] [10. Student Do: Users and Groups Activity](LessonPlan.md#10-student-do-users-and-groups-activity-020)
- [x] [11. Instructor Review: Users and Groups](LessonPlan.md#11-instructor-review-users-and-groups-010)
...
:question: **Ask class**: What makes a stronger password? Its complexity or length?
...
- Enter `jjjjjjjj` (8 js). Mention that 8 characters crack instantly.
...
- If a system requires 1 character and nothing else, the password will remain relatively strong, even if words are embedded.
...
- Explain the two steps needed to run John the Ripper:
...
- Explain that you can also have John the Ripper run against a pre-defined wordlist to speed up the cracking process with the following format:
...
- Edit the copied shadow file to only include several specific users; and
...
2. Use Nano to edit your `shadow_copy` file to only leave the rows for the following users you will crack:
    ...
    - **Solution**:
      ...
      - Run `sudo nano shadow_copy` and delete all extra lines that are not the users in the preceding list.
...
- `sudo` can also control which commands the user can run as root user. This way, the system has granular control over who can run root commands, and which ones. It also keeps a log of exactly which user runs which commands (this can be reviewed as needed).

Explain how to configure `sudo` access:

- `sudo` access is configured using a configuration file called the `sudoers` file.
...
Mention that once you, or a process, are logged in as `root`, though, you can make any changes you want to the system, including changes or malicious actions that may harm the system.
...
Explain that in order to update the `/etc/sudoers` file, you must use the command `visudo`, which opens the `etc/sudoers` file using Nano.
  ...
  - :warning: **Heads Up:**  If students break this file and gets locked out of using `sudo`, they should review the following thread: [Ask Ubuntu: How to Modify an Invalid etc Sudoers File](https://askubuntu.com/questions/73864/how-to-modify-an-invalid-etc-sudoers-file).
...
To demonstrate this, explain that we will use `sudo less` with the admin user and then go from `less` directly into a root shell with `!bash`.
...
We now have a `#` at the prompt again, indicating that we have `root` privileges.
...
- Therefore, a sysadmin must know how to add and remove users from a system, add and remove groups, and add and remove users from those groups.
...
- Run `sudo usermod -aG developers joseph` and break down the syntax once more.
  ...
  - `developers`: Following the `-aG` flag are the groups we wish to add the user to. In this case, we want to add the user `joseph` to the `developers` group.
